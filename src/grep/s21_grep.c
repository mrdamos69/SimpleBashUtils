...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        (strlen(*(argv + i)) < 5); j++ ) {
            ...
            if (argv[i][1] != '-') {
                switch (menu) {
                    case 'e': spec_file.min_e = 1; break;
                    case 'i': spec_file.min_i = 1; break;
                    case 'v': spec_file.min_v = 1; break;
                    case 'c': spec_file.min_c = 1; break;
                    case 'l': spec_file.min_l = 1; break;
                    case 'n': spec_file.min_n = 1; break;
                    case 'f': spec_file.min_f = 1; break;
                    case 'h': spec_file.min_h = 1; break;
                    case 'o': spec_file.min_o = 1; break;
                    case 's': spec_file.min_s = 1; break;
                    default: exit = i; break;
                }
            }
        }
        if ((argv[i][0] != '-' && flag)) {
            if (spec_file.min_f == 1) {
                ...
                if (file_reg != NULL) {
                    ...
                    !spec_file.min_s ? printf("grep: %s: No such file or directory\n", *(argv + i)) :
                    spec_file.min_s;
                    exit = 7;
                    break;
                }
     ...
     if ((file_name[0] != '-') && (strcmp(spec_file.reg_name, file_name))) {
        if (file != NULL) {
            while (getline(&buffer, &len, file) != -1) {
                ...
                spec_file.min_l ? spec_file.min_l = (!reti ? 2 : spec_file.min_l) : spec_file.min_l;
                size_line++;
            }
            free(buffer);
            if (spec_file.min_l) {
                size_temp = (size_temp > 0 ? 1 : 0);
                spec_file.min_c ? printf("%s%d\n", temp_name, size_temp) : spec_file.min_c;
            } else {
                spec_file.min_c ? printf("%s%d\n", temp_name, size_temp) : spec_file.min_c;
            }
            spec_file.min_l == 2 ? printf("%s\n", file_name) : spec_file.min_l;
        } else {
            !spec_file.min_s ? printf("grep: %s: No such file or directory\n", file_name) : spec_file.min_s;
        }
        fclose(file);
    }
}

void spec_v_and_o(grep_spec spec_file, char* buffer, char* temp_name, int *reti, int *size_line) {
        ...
        if (spec_file.min_n == 0) {
            if (spec_file.min_v) {
                *reti ? printf("%s%s", temp_name, buffer) : *reti;
                ...
                if (!(*reti)) {
                    !spec_file.min_o ? !(*reti) ? printf("%s%s", temp_name, buffer) :
                    *reti : !spec_file.min_o;
            ...
            if (spec_file.min_v) {
                *reti ? printf("%s%d:%s", temp_name, *size_line, buffer) : *reti;
                ...
                if (!(*reti)) {
                    !spec_file.min_o ? !(*reti) ? printf("%s%d:%s", temp_name, *size_line, buffer) :
                    *reti : !spec_file.min_o;
                    spec_file.min_o ? printf("%s%d:%s\n", temp_name, *size_line,
                    spec_file.reg_name) : spec_file.min_o;
                }
            }
        }
    }
